...
from yolken_test2 import YolkenTest2, AsyncYolkenTest2, DefaultAioHttpClient
from yolken_test2._utils import is_dict
...
logging.getLogger("yolken_test2").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[YolkenTest2]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with YolkenTest2(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncYolkenTest2]:
